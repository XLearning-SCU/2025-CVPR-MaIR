...
class LoSh2D(nn.Module):
    def __init__(
        ...
        self.gating1 = nn.Sequential(
            ...
            nn.Conv2d(self.d_inner, 1, kernel_size=1, stride=1, padding=0),
            nn.Sigmoid()
        )
    ...
    def forward(self, x: torch.Tensor, losh_ids, **kwargs):
        ...
        y1, y2, y3, y4 = y1.permute(0, 3, 1, 2).contiguous(), y2.permute(0, 3, 1, 2).contiguous(), y3.permute(0, 3, 1, 2).contiguous(), y4.permute(0, 3, 1, 2).contiguous()
        y1 = y1 * self.gating1(y1)
        y2 = y2 * self.gating2(y2)
        y3 = y3 * self.gating3(y3)
        y4 = y4 * self.gating4(y4)
        y = y1 + y2 + y3 + y4
        # y = y.reshape
...
if __name__ == '__main__':
    ...
    net = LnMLPGcs(img_size=(64, 64), embed_dim=60, d_state=1, ssm_ratio=1.0, dynamic_ids=False, batch_size=1,mlp_ratio=1.5).cuda()
