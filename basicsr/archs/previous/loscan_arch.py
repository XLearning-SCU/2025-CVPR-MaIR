...
class LoSh2D(nn.Module):
    ...
    def forward_core(self, x: torch.Tensor, losh_ids):
        ...
        # torch.cuda.synchronize()
        # end = time.time()
        # print("losh_ids_scan time:", (end-start)*24, " s")
...
class LoScanNet(nn.Module):
    r""" LoScanNet Model
           A PyTorch impl of : `A Simple Baseline for Image Restoration with State Space Model `.

           Already implement the S-Shape Scan, do not implement the Shift Window
...
if __name__ == '__main__':
    ...
    net = LoScanNet(img_size=64, dynamic_ids=False, batch_size=1).cuda()
    # inp = torch.tensor
