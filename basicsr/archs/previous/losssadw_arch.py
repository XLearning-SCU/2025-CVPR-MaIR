...
# try:
from basicsr.archs.shift_scan_util import losh_ids_generate, losh_ids_scan, losh_ids_inverse, losh_shift_ids_generate, losh_separately_shift_ids_generate
# except:
#     from shift_scan_util import losh_ids_generate, losh_ids_scan, losh_ids_inverse, losh_shift_ids_generate, losh_separately_shift_ids_generate
...
class LoSSS2D(nn.Module):
    def __init__(
        ...
        self.d_mamba = self.d_inner // 4 # channels relative to mamba block
        ...
        self.x_proj = (
            nn.Linear(self.d_mamba, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs),
            nn.Linear(self.d_mamba, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs),
            nn.Linear(self.d_mamba, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs),
            nn.Linear(self.d_mamba, (self.dt_rank + self.d_state * 2), bias=False, **factory_kwargs),
        )
        ...
        self.dt_projs = (
            self.dt_init(self.dt_rank, self.d_mamba, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                         **factory_kwargs),
            self.dt_init(self.dt_rank, self.d_mamba, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
                         **factory_kwargs),
            self.dt_init(self.dt_rank, self.d_mamba, dt_scale, dt_init, dt_min, dt_max, dt_init_floor,
        ...
        self.A_logs = self.A_log_init(self.d_state, self.d_mamba, copies=4, merge=True)  # (K=4, D, N)
        self.Ds = self.D_init(self.d_mamba, copies=4, merge=True)  # (K=4, D, N)
        ...
        # self.simple_sa = nn.Sequential(
        #     nn.Conv2d(in_channels=self.d_inner, out_channels=4, kernel_size=3, padding=1, stride=1,
        #               groups=4, bias=True),
        #     nn.SiLU()
        # )
        # self.simple_sa = nn.Sequential(
        #     nn.Conv2d(in_channels=self.d_inner, out_channels=4, kernel_size=1, padding=0, stride=1,
        #               groups=1, bias=True),
        #     nn.Conv2d(in_channels=4, out_channels=4, kernel_size=3, padding=1, stride=1,
        #               groups=4, bias=True),
        #     nn.SiLU()
        # )
        # 原本: d_inner * d_inner * 9
        # 现在: d_inner/g * d_inner/g * g, 原本的1/g 
    ...
    def forward_core(self, x: torch.Tensor, 
        ...
        # x = x.reshape(B*4, C//4, C, H)
        xs = losh_ids_scan(x, xs_scan_ids)
        #  S
        # xs = xs.reshape(B*self.)
        # input (B, C, H, W)
    ...
    def forward(self, x: torch.Tensor, losh_ids, **kwargs):
        ...
        # y = y1 + y2 + y3 + y4
        # y_cat = torch.cat((y1, y2, y3, y4), dim=1).reshape(B, -1, H, W)
        # y = torch.transpose(y, dim0=1, dim1=2).contiguous().view(B, -1, H, W)
        # sa_map = self.simple_sa(y_cat).reshape(B, 4, -1)

        # sa_map = self.simple_sa(x).reshape(B, 4, -1)
        # y1, y2, y3, y4 = y1 * sa_map[:,0:1,:], y2 * sa_map[:,1:2,:], y3 * sa_map[:,2:3,:], y4 * sa_map[:,3:4,:]
        
        y = torch.cat((y1, y2, y3, y4), dim=1).reshape(B, H, W, -1)
        # TODO: 如果这样效果不佳，试一下把transpose加上，是不是好点？
        # y = (y * self.simple_sa(y)).reshape(B, H, W, -1)
    ...
    def flops_forward_core(self, H, W):
        ...
        flops += 4 * 9 * H * W * self.d_mamba * self.d_state
    ...
    def flops(self):
        ...
        # 64 64 16 360
        # y = y1 + y2 + y3 + y4 -> y = torch.cat((y1, y2, y3, y4), dim=1)
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.self_attention = LoSSS2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, input_resolution=input_resolution, **kwargs)
...
class LoSSSADW(nn.Module):
    ...
    def _generate_ids(self, inp_shape):
        ...
        xs_scan_ids, xs_inverse_ids = losh_separately_shift_ids_generate(inp_shape=(B, int(C*self.mlp_ratio), H, W), 
                                                                         scan_len=self.scan_len, shift_len=0)# [B,H,W,C]
        ...
        xs_shift_scan_ids, xs_shift_inverse_ids = losh_separately_shift_ids_generate(inp_shape=(B, int(C*self.mlp_ratio), H, W), scan_len=self.scan_len, shift_len=self.scan_len//2) # [B,H,W,C]
    ...
    def forward_features(self, x):
        ...
        if self.dynamic_ids or (not self.training):
            ...
            xs_scan_ids, xs_inverse_ids = losh_separately_shift_ids_generate(inp_shape=(B, int(C*self.mlp_ratio), H, W), 
                                                                             scan_len=self.scan_len,shift_len=0)# [B,H,W,C]
            xs_shift_scan_ids, xs_shift_inverse_ids = losh_separately_shift_ids_generate(inp_shape=(B, int(C*self.mlp_ratio), H, W), 
                                                                                         scan_len=self.scan_len, shift_len=self.scan_len//2)# [B,H,W,C]
        else:
...
if __name__ == '__main__':
    ...
    net = LoSSSADW(img_size=(64, 64), embed_dim=60, d_state=1, mlp_ratio=1.4, dynamic_ids=False, batch_size=1).cuda()
