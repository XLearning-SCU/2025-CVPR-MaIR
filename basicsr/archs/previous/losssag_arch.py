...
class LoSSGating2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, losh_ids, **kwargs):
        ...
        y1 = y1.view(B, -1, H, W).contiguous()
        y2 = y2.view(B, -1, H, W).contiguous()
        y3 = y3.view(B, -1, H, W).contiguous()
        y4 = y4.view(B, -1, H, W).contiguous()

        y_cat = torch.cat((y1,y2,y3,y4), dim=1)

        y_sag_cat = y_cat.view(B, 4, -1, H, W).contiguous()
        y_sag_cat = torch.cat((torch.max(y_sag_cat, 2)[0], torch.mean(y_sag_cat, 2)), dim=1)

        attn_sag = self.sag(y_sag_cat)
        y_sag = y1 * attn_sag[:, 0:1] + y2 * attn_sag[:, 1:2] + y3 * attn_sag[:, 2:3] + y4 * attn_sag[:, 3:4] 
        y = y_sag.view(B, H, W, -1).contiguous()
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.self_attention = LoSSGating2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, **kwargs)
...
if __name__ == '__main__':
    ...
    net = LoSSSAG(img_size=64, dynamic_ids=False, batch_size=1).cuda()
    # inp = torch.tensor
