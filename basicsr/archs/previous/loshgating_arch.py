...
from basicsr.archs.scan_util import losh_ids_generate, losh_ids_scan, losh_ids_inverse
# from scan_util import losh_ids_generate, losh_ids_scan, losh_ids_inverse
# from basicsr.archs.scan_util import loshscan, losh_decoup
...
class LoShGating2D(nn.Module):
    def __init__(
        ...
        self.gating = nn.Linear(4*self.d_inner, 4, bias=bias, **factory_kwargs)
        self.softmax = nn.Softmax(dim=-1)
    ...
    def forward(self, x: torch.Tensor, losh_ids, **kwargs):
        ...
        y3_mean = torch.mean(y3, dim=-1)
        y4_mean = torch.mean(y4, dim=-1)
        out_mean = torch.cat([y1_mean, y2_mean, y3_mean, y4_mean], dim=-1)
        gating_coeff = self.softmax(self.gating(out_mean))
        gating_coeff = gating_coeff.unsqueeze(-1)
        y = gating_coeff[:, 0:1] * y1 + gating_coeff[:, 1:2] * y2 + gating_coeff[:, 2:3] * y3 + gating_coeff[:, 3:4] * y4
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.self_attention = LoShGating2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, **kwargs)
...
class LoShGating(nn.Module):
    r""" LoShSwin Model
           A PyTorch impl of : `A Simple Baseline for Image Restoration with State Space Model`.
           Replace the out stucture of Mamba by the structure of the Swin.
    ...
    def __init__(self,
        ...
        super(LoShGating, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
    ...
    def forward_features(self, x):
        ...
        # end = time.time()
        # print("losh_ids_generate time:", (end-start), " s")

        # xs_scan_ids, xs_inverse_ids = losh_ids_generate(inp_shape=(B, int(C*self.mlp_ratio), H, W), scan_len=4)# [B,H,W,C]
        

        for layer in self.layers:

            x = layer(x, (xs_scan_ids, xs_inverse_ids), x_size)
...
if __name__ == '__main__':
    ...
    net = LoShGating(img_size=64, dynamic_ids=False, batch_size=1).cuda()
    # inp = torch.tensor
    ...
    # print(ret_dict['layers'])
    # for key in ret_dict.keys():
    #     # print("Name: %15s, Flops: %10f M, Params: %10f M"&(key, ret_dict[key][0]/1e6, ret_dict[key][1]/1e3))
    #     print("Name: %-20s, Flops: %-8.2f M, Per: %-5.2f %%, Params: %-8.2f K, Per: %5.2f %%"%(key, ret_dict[key][0]/1e6, ret_dict[key][0]/ flops *100,
    #                                                                                      ret_dict[key][1]/1e3, ret_dict[key][1] / params * 100))
