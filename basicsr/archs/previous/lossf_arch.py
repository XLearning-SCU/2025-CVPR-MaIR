...
class LoSh2D(nn.Module):
    def __init__(
        ...
        self.in_proj = Linear2d(self.d_model, self.d_inner * 2, bias=bias, **factory_kwargs)
    ...
    def forward_core(self, x: torch.Tensor, losh_ids):
        ...
        xs = losh_ids_scan(x, xs_scan_ids, bkdl=True)
        ...
        y1, y2, y3, y4 = losh_ids_inverse(out_y, xs_inverse_ids, (B, C, H, W))
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.self_attention = LoSh2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate,input_resolution=input_resolution,**kwargs)
...
if __name__ == '__main__':
    ...
    net = LoSSFNet(img_size=(640, 360), d_state=1, dynamic_ids=False, batch_size=1).cuda()
    ...
    params = fvcore_parameter_count(net)[""]
    print("Params by fvcore: ", params/1e6 , "M")
