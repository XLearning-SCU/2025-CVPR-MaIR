...
class LoSh2D(nn.Module):
    ...
    def forward(self, x: torch.Tensor, losh_ids, **kwargs):
        ...
        y1 = y1 * self.gating1(x)
        y2 = y2 * self.gating2(x)
        y3 = y3 * self.gating3(x)
        y4 = y4 * self.gating4(x)
        y = y1 + y2 + y3 + y4
        # y = y.reshape
...
if __name__ == '__main__':
    ...
    net = LnMLPGx(img_size=(64, 64), embed_dim=60, d_state=1, ssm_ratio=1.0, dynamic_ids=False, batch_size=1,mlp_ratio=1.5).cuda()
