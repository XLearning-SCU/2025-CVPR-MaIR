...
class LoSh2D(nn.Module):
    def __init__(
        ...
        #     nn.AdaptiveAvgPool2d(1),
        #     # nn.Conv2d(self.d_inner*4, self.d_inner, groups=4, kernel_size=1, stride=1, padding=0),
        #     nn.Conv2d(self.d_inner*4, 4, groups=4, kernel_size=1, stride=1, padding=0),
        #     nn.Sigmoid()
        # )
        ...
        #     nn.AdaptiveAvgPool2d(1),
        #     # nn.Conv2d(self.d_inner*4, self.d_inner, groups=4, kernel_size=1, stride=1, padding=0),
        #     nn.Conv2d(self.d_inner*4, self.d_inner*4, groups=self.d_inner, kernel_size=1, stride=1, padding=0),
        #     nn.Sigmoid()
        # )
    ...
    def forward_core(self, x: torch.Tensor, 
        ...
        y1, y2, y3, y4 = losh_ids_inverse(out_y, xs_inverse_ids) #B, C, L
...
class VSSBlock(nn.Module):
    ...
    def forward(self, input, losh_ids, x_size):
        ...
        x = x*self.skip_scale2 + self.drop_path(self.conv_blk(self.ln_2(x)))
...
class LnMLPSGwDPR(nn.Module):
    ...
    def __init__(self,
        ...
        super(LnMLPSGwDPR, self).__init__()
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
    ...
    def _generate_ids(self, inp_shape):
        ...
        xs_scan_ids, xs_inverse_ids = losh_ids_generate(inp_shape=(B, int(C*self.ssm_ratio), H, W), scan_len=self.scan_len)# [B,H,W,C]
        ...
        xs_shift_scan_ids, xs_shift_inverse_ids = losh_shift_ids_generate(inp_shape=(B, int(C*self.ssm_ratio), H, W), scan_len=self.scan_len, shift_len=self.scan_len//2)# [B,H,W,C]
        if torch.cuda.is_available():
...
if __name__ == '__main__':
    ...
    net = LnMLPSGwDPR(img_size=(64, 64), embed_dim=60, d_state=1, ssm_ratio=1.2, dynamic_ids=False, batch_size=1,mlp_ratio=1.5).cuda()
    # net = LnMLPG(img_size=(64, 64), depths=(6, 6, 6, 6, 6, 6), embed_dim=180, d_state=16, ssm_ratio=2.0, dynamic_ids=False, batch_size=1,mlp_ratio=2.0, upsampler='pixelshuffle').cuda()
    net.train()
